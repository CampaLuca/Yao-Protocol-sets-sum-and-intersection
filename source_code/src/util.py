...
# taken from: https://stackoverflow.com/questions/104420/how-to-generate-all-permutations-of-a-list
...
# get a single permutation for Bob's usage
def get_single_permutation(elements):
    int_list = elements

    if len(elements) > 5:
        for i in range(0, len(elements)-4):
            if i == len(elements)-5:
                int_list = int_list[:i] + list(all_perms(int_list[i:]))[random.randint(0, 120)]
            else:
                int_list = int_list[:i] + list(all_perms(int_list[i:i+5]))[random.randint(0, 120)] + int_list[i+5:]
    else:
        int_list = list(all_perms(int_list))[random.randint(0, math.factorial(len(int_list))-1)]
